...
setwd("C:/Users/Tim/Desktop/Bioinformatics_master/RNA_SEQ/Datasets/codon_occupancy")
...
## Null hypothesis: codon speed does not differ between wt and ko

qqPlot(data_plot$Norm_Occupancy_KO) ## not normally distributed but close (log transformation makes it better but does not fix it completely)
t.test(data_plot$Norm_Occupancy_KO-1) ## mean is not significantly different from 1
occupancy_sd<-sd(data_plot$Norm_Occupancy_KO)
occupancy_mean<-mean(data_plot$Norm_Occupancy_KO)
critical_value <- qnorm(1 - 0.025) ## while data is not normal, normality is assumed for the interval
upper_tolerance<- occupancy_mean+critical_value*occupancy_sd
...
ggplot(data_plot,
  ...
  ylab("Codons") +
  geom_rect(xmin = 0, xmax = upper_tolerance, ymin = -Inf, ymax = Inf, fill = "purple3",alpha=0.009) +
  geom_vline(xintercept = occupancy_mean, color = "purple", linetype = "dashed") +
  theme_bw()
